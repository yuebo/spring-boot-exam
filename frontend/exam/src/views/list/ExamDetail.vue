  ...
  <a-layout>
    <a-layout-header class="header" style="color: #fff">
      ...
      <span style="float: right;">
        <span style="margin-right: 60px; font-size: 20px" v-if="examDetail.exam">考试限时：{{ examDetail.exam.examTimeLimit }}分钟 {{timeLeft}}</span>

        <a-button type="danger" ghost style="margin-right: 60px;" v-if="!examRecord.examRecordId" @click="startExam()">开始</a-button>
        <a-button type="danger" ghost style="margin-right: 60px;" v-else @click="finishExam()">交卷</a-button>
    ...
    <a-layout>
      <a-layout-sider width="190" :style="{background: '#444',overflow: 'auto', height: '100vh', position: 'fixed', left: 0 }">
        ...
        >
          <template  v-if="examRecord.examRecordId">
          <a-sub-menu key="question_radio" v-if="examDetail.radioIds.length>0">
          ...
          <a-sub-menu key="question_check" v-if="examDetail.checkIds.length>0">
          ...
          <a-sub-menu key="question_judge" v-if="examDetail.judgeIds.length>0">
        ...
        <a-layout-content :style="{ margin: '24px 16px 0',height: '84vh', overflow: 'initial' }">
          <div :style="{ padding: '24px', background: '#fff',height: '84vh'}">
            <span v-show="currentQuestion === ''" style="font-size: 30px;font-family: Consolas">
              欢迎参加考试，请点击上方开始按钮开始答题，答题完毕请及时交卷。
            </span>
            <strong>{{ currentQuestion.type }} </strong> {{ currentQuestion.name }}
            <br><br>
            <!-- 单选题和判断题 --> <!-- key不重复只需要在一个for循环中保证即可 -->
            <a-radio-group :disabled="timeLeft=='已超时'" @change="onRadioChange" v-model="radioValue" v-if="currentQuestion.type === '单选题' || currentQuestion.type === '判断题'">
            ...
            <!-- 多选题 -->
            <a-checkbox-group :disabled="timeLeft=='已超时'" @change="onCheckChange" v-model="checkValues" v-if="currentQuestion.type === '多选题'">
...
<script>
import { getExamDetail, getQuestionDetail, finishExam, startExam, getLastExamRecord, saveExamRecord } from '../../api/exam'
...
export default {
  ...
  computed: {
    timeLeft () {
      if (!this.examRecord.examRecordId) {
        return '尚未开始'
      }
      if (this.examRecord.examJoinDateTimestamp / 1000 + this.examDetail.exam.examTimeLimit * 60 - this.currentTimestamp>=0){
        return `剩余${Math.floor((this.examRecord.examJoinDateTimestamp / 1000 + this.examDetail.exam.examTimeLimit * 60 - this.currentTimestamp) / 60)}分钟`
      } else {
        return '已超时'
      }
    }
  ...
  mounted () {
    ...
    getExamDetail(this.$route.params.id)
      .then(res => {
        if (res.code === 0) {
          ...
          // 继续上次考试
          getLastExamRecord(this.$route.params.id).then(r => {
            if (r.code === 0) {
              this.examRecord = r.data || {}
              if (r.data) {
                this.answersMap = this._objToStrMap(r.data.answer != null ? JSON.parse(r.data.answer) : {})
              }
            }
          })
    ...
    const fn = () => {
      setTimeout(() => {
        this.currentTimestamp = Date.parse(new Date()) / 1000
        fn()
      }, 1000)
    }
    fn()
  ...
  methods: {
    ...
    onAnswerChange () {
      saveExamRecord(this.examRecord.examRecordId, this._mapToJson(this.answersMap)).then(res => {
        if (res.code !== 0) {
    ...
    startExam () {
      startExam(this.$route.params.id).then(res => {
        if (res.code === 0) {
    ...
    finishExam () {
      ...
      finishExam(this.examRecord.examRecordId, this._mapToJson(this.answersMap))
        .then(res => {
          if (res.code === 0) {
