    ...
    <a-card
      ...
      <a-list size="large">
        <a-list-item :key="index" v-for="(item, index) in data">
          ...
          <div slot="actions">
            <a @click="viewExamRecordDetail(item.examRecord)" v-if="item.examRecord.examJoinScore>=0">查看考试详情</a>
            <a @click="viewExam(item.examRecord)" v-else>继续考试</a>
          </div>
          <div class="list-content">
            ...
            <div class="list-content-item">
              <span>分数</span>
              <p>{{ item.examRecord.examJoinScore<0? '未完成': item.examRecord.examJoinScore}}</p>
            </div>
          </div>
        </a-list-item>
      </a-list>
